...
class SqlangParser():
    ...
    # 递归地遍历SQL解析树中的所有令牌，对字符串类型的令牌进行处理
    ...
    # 递归地遍历SQL解析树中的所有令牌，并为表、列、浮点数、整数和十六进制数重新命名，以便后续处理
    ...
    # 为对象生成一个哈希值，基于对象的 tokensWithBlanks 属性
    def __hash__(self):
        return hash(tuple([str(x) for x in self.tokensWithBlanks]))

    #构造函数，初始化 SqlangParser 对象。对输入的SQL进行预处理和解析。执行一系列操作，包括移除空白、识别字面量、子查询、函数和表等。
...
#######################主函数：句子的tokens##################################
# 将输入的SQL语句或文本进行标准化和分词处理，用了更严格的字符过滤
...
# 将输入的SQL语句或文本进行标准化和分词处理，保留了更多的特殊字符
